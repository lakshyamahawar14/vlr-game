...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const updateCount = async () => {
      const { data } = await supabase.from("queue").select("id");
      setOnlinePlayers(data?.length || 0);
    };
    ...
    channel.on(
      ...
      () => setOnlinePlayers((prev) => Math.max(prev - 1, 0))
    );
    ...
    <div className="flex min-h-screen bg-white text-black font-mono">
      <div className="flex-1 flex flex-col items-center justify-center p-10">
        ...
        <div className="mt-2 text-sm font-bold uppercase text-gray-600">
          Players in Queue: {onlinePlayers}
        </div>
      ...
      <div className="w-80 border-l-4 border-black p-8 flex flex-col bg-gray-50">
        ...
        <ul className="space-y-6">
          <li className="flex gap-4"><span className="font-black text-xl">01</span><p className="text-sm font-bold uppercase leading-tight">Start with a $100 budget to build your pro team.</p></li>
          <li className="flex gap-4"><span className="font-black text-xl">02</span><p className="text-sm font-bold uppercase leading-tight">You must select exactly 5 players before time runs out.</p></li>
          <li className="flex gap-4"><span className="font-black text-xl">03</span><p className="text-sm font-bold uppercase leading-tight">Players are unique. If your opponent picks them first, they are gone.</p></li>
          <li className="flex gap-4"><span className="font-black text-xl">04</span><p className="text-sm font-bold uppercase leading-tight">You have 30 seconds to complete your roster.</p></li>
          <li className="flex gap-4"><span className="font-black text-xl">05</span><p className="text-sm font-bold uppercase leading-tight">Disconnecting results in an automatic forfeit.</p></li>
        </ul>
